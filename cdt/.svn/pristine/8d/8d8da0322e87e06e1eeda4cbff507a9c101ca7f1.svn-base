$(document).ready(function () {
    ...
    //kiểm tra màn hình
    if (eval(textUtils.getCookie('popIndex')) <= 1) {
        $("div[rel=popIndex]").css({"display": "block"});
    }
    $("div[rel=popIndex] span.pm-close").click(function () {
        $("div[rel=popIndex]").css({"display": "none"});
    });
    ...
    $(".trans-item").click(function () {
        ...
        if ($(parent).hasClass("active")) {
            ...
            $('.trans-radio', $(this)).attr('checked', false);
        } else {
            $('li.active .content-trans', $(grand)).slideUp();
            $('.trans-radio', $(this)).attr('checked', true);
    ...
    $('.btn-more-button').click(function () {
        var parent = $(this).parent();
        if ($('.shipping-block-statics').hasClass("height-toggle")) {
            $('.shipping-block-statics').removeClass("height-toggle");
        } else {
            $('.shipping-block-statics').addClass("height-toggle");
            $('.shipping-block-statics').fadeIn();
        }
        return true;
    });
