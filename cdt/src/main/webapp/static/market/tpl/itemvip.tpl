<div class="product-item">
    <div class="big-shadow">
        ...
        <%}%>
        <a class="view-now" href="<%= baseUrl %>/san-pham/<%= data.id %>/<%= textUtils.createAlias(data.name) %>.html">Xem ngay</a>
        <div class="item-img"><a href="<%= baseUrl %>/san-pham/<%= data.id %>/<%= textUtils.createAlias(data.name) %>.html"><img src="<%=(typeof data.images != 'undefined' && data.images.length >0)?data.images[0]:staticUrl+'/market/images/no-image-product.png'%>"  alt="<%=data.name%>" title="<%=data.name%>"/></a></div>
        <div class="item-text">
            ...
            <div class="item-row bg-item">
                <div class="item-shop">
                    ...
                    <a href="<%= baseUrl%>/<%= shops[i].alias %>/"><%= shops[i].alias %></a>

                    <%}}
                    if(!hasShop){
                    %>
                    <a href="<%= urlUtils.browseUrl(itemSearch, '', [{key: 'cid', op: 'rm'},{key:'sellerId',val:data.sellerId,op:'mk'}]) %>/"><%=data.sellerName%></a>
                    <%}%>
